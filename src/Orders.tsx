...
export default function Orders() {
  ...
  const [mode, setMode] = useState<FloorMode>("view");
  ...
  const currentlySelectedTable = tables.find(
  ...
  const getGroupTables = (table: TableData) => {
    if (!table.groupId) {
      return [table];
    }

    return tables.filter(
      (item) => item.groupId === table.groupId
    );
  };

  const getOrderTableName = (order: OrderGroup) => {
    return tables
      .filter((table) =>
  ...
  const handleTableClick = (table: TableData) => {
    ...
    if (existingOrder) {
      setGuestCount(existingOrder.guests);
    } else {
      setGuestCount(1);
    }
  };
  ...
  const deleteSelectedTable = () => {
    ...
    const active =
      getOrderForTable(selectedTableId);
  ...
  const joinSelectedTables = () => {
    ...
    setTables((current) =>
      current.map((table) =>
        joinSelection.includes(table.id)
          ? {
              ...table,
              groupId,
            }
  ...
  const unjoinSelectedTables = () => {
    ...
    const groupIds = tables
      .filter((table) =>
        joinSelection.includes(table.id)
      )
      .map((table) => table.groupId)
      .filter(
    ...
    const groupTableIds = tables
      .filter(
        (table) =>
          table.groupId &&
          groupIds.includes(table.groupId)
      )
      .map((table) => table.id);

    const hasOpenOrder =
      groupTableIds.some((tableId) =>
        Boolean(
          getOrderForTable(tableId)
        )
      );
  ...
  return (
    ...
    >
      ...
      >
        ✓ Floor Plan and Order Groups save automatically
      </div>
